...
//   2) bettingSites / casinoSites `.link` - direct product domain (fallback)
//   3) horizontalBanners / horizontalCasinoBanners `.link` - banner fallback
...
function resolveTarget(slug: string): string | null {
  ...
  const site = casinoSites.find((s) => s.slug === slug)
  if (site?.link) return site.link.trim()

  const banner = horizontalCasinoBanners.find((b) => b.slug === slug)
